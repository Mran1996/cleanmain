export function KeyFeatures() {
  return (
    <section className="bg-white py-20">
      <div className="max-w-5xl mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold mb-6">Ask AI Legal's Key Features</h2>
        </div>

        <div className="grid grid-cols-2 gap-x-16 gap-y-12 max-w-5xl mx-auto">
          <div className="flex items-start gap-4">
            <div className="flex-shrink-0 w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center">
              <span role="img" aria-label="Scale" className="text-2xl">
                ⚖️
              </span>
            </div>
            <div>
              <h3 className="text-xl font-medium text-emerald-600 mb-2">Precision-Trained AI</h3>
              <p className="text-base text-gray-600">
                Trained on millions of real legal documents, Ask AI Legal remembers facts, spots key issues, and builds smart, strategic responses — instantly.
              </p>
            </div>
          </div>

          <div className="flex items-start gap-4">
            <div className="flex-shrink-0 w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center">
              <span role="img" aria-label="Brain" className="text-2xl">
                🧠
              </span>
            </div>
            <div>
              <h3 className="text-xl font-medium text-emerald-600 mb-2">Smart Legal Intake</h3>
              <p className="text-base text-gray-600">
                It asks the right questions, in the right order — guiding you through the facts it needs to build your strongest possible legal draft.
              </p>
            </div>
          </div>

          <div className="flex items-start gap-4">
            <div className="flex-shrink-0 w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center">
              <span role="img" aria-label="Pen" className="text-2xl">
                ✍️
              </span>
            </div>
            <div>
              <h3 className="text-xl font-medium text-emerald-600 mb-2">Court-Ready Drafts</h3>
              <p className="text-base text-gray-600">
                Get fully formatted legal documents — motions, letters, complaints, or responses — ready to file, edit, or send.
              </p>
            </div>
          </div>

          <div className="flex items-start gap-4">
            <div className="flex-shrink-0 w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center">
              <span role="img" aria-label="Upload" className="text-2xl">
                📤
              </span>
            </div>
            <div>
              <h3 className="text-xl font-medium text-emerald-600 mb-2">Intelligent Document Upload</h3>
              <p className="text-base text-gray-600">
                Upload your motion, complaint, or charge sheet. Ask AI Legal will read it, pull out the key info, and tailor your legal response around it.
              </p>
            </div>
          </div>

          <div className="flex items-start gap-4">
            <div className="flex-shrink-0 w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center">
              <span role="img" aria-label="Book" className="text-2xl">
                📚
              </span>
            </div>
            <div>
              <h3 className="text-xl font-medium text-emerald-600 mb-2">Real Case Law + Citations</h3>
              <p className="text-base text-gray-600">
                Includes citations and case law that match your facts — powered by advanced legal search tools to strengthen your position.
              </p>
            </div>
          </div>

          <div className="flex items-start gap-4">
            <div className="flex-shrink-0 w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center">
              <span role="img" aria-label="Clock" className="text-2xl">
                🕒
              </span>
            </div>
            <div>
              <h3 className="text-xl font-medium text-emerald-600 mb-2">Always-On Legal Help</h3>
              <p className="text-base text-gray-600">
                Ask AI Legal works 24/7. Whether you're in court tomorrow or just got served — we're here when you need fast answers and real help.
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}
