...
BEGIN
  ...
  ELSE
    ...
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'chat_conversations' AND column_name = 'metadata') THEN
      ALTER TABLE chat_conversations ADD COLUMN metadata JSONB DEFAULT '{}'::jsonb;
    END IF;
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    ...
    -- Ensure missing columns exist when table already exists
    IF NOT EXISTS (
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'chat_messages' 
      AND policyname = 'Users can insert messages in their conversations'
  ) THEN
